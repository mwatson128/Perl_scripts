...
$workdir = "/uswsup01/research/per_all/";
...
sub copy_files {
  ...
  $logdir = "/uswsup01/loghist/per_all/";
  
    # Build command to get per file 
    $SCP_PER = sprintf "/bin/cp %s%s* %s", 
         $logdir, $perfile, $workdir;
    $DBUG && print("SCP command:$SCP_PER\n");

    # Get the per file
    system $SCP_PER;

  qx(gunzip $workdir$perfile*);
...
sub update_perf {
  ...
  $UPDATE_PERF = sprintf "cat %sperf.update >> %s%s", $workdir,
                         $workdir, $perf_file;

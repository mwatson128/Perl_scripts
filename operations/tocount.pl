#!/usr/local/bin/perl
# (]$[) tocount.pl:1.8 | CDATE=07/14/09 11:05:01
######################################################################
# This script goes through the runtime log and pulls out the context
# timeouts and dumps an hourly breakdown for each HRS by GDS, for 
...
$zone = `uname -n`;
chomp $zone;
$log_dir = "/uswdev/mwatson_dev/";
#$to_dir = "/$zone/usw/reports/daily/tologs/";
$to_dir = "/uswdev/mwatson_dev/tologs/";
  ...
  if ($line !~ /^$|^#/) {
    ...
    if ($line =~ /{/) {
      ...
      if ($config_type eq "HRS_EQUIVALENCE") {
        ...
        ($prim_gds, @rest) = split /\s/, $hold{GDS};
        $gds_conn{$prim_gds} = $hold{HRS};
      }
    }
  }
}
...
$tologfile = $to_dir . $month . $year . "/tolog" . $day;

# Create the month dir, and if it errors, just ignore it.
`/bin/mkdir $to_dir/$month$year 2>&1 > /dev/null`;

# Close STDOUT and open it as the tolog file.  Comment out these two
# lines if you want to run this to the display.
close STDOUT;
open STDOUT, "> $tologfile" or die "Can't open tolog file $tologfile\n";
...
while ($block = <RTLOG>) {
  ...
  if ($block =~ /CTTM/) {                     
    ...
    for $line (@block) {
      ...
      # Double check to see if its a CTTM
      if ($line =~ /CTTM/) { 
...
for $hrs (@hrslist) {
  ...
  while ($hour < 24) {
    ...
    $total = 0;
    for $gds (@gdslist) {
      printf"  %5d", $hrsto{$hrs}{$gds}[$hour]; 
      $total = $total + $hrsto{$hrs}{$gds}[$hour];
      $total{$gds} = $total{$gds} + $hrsto{$hrs}{$gds}[$hour];
    } 
    printf "  %5s\n", $total;
    $hour++;
  }
...
#################################
%gdstotal=();
for $gds (sort keys %gds_conn) {

  print "in GDS looking at $gds \n";
  print "GDS list of hrs: $gds_conn{$gds}\n";
  $hrslist = $gds_conn{$gds};
  @hrslist = split (/\s/), $gds_conn{$gds};
  $hrsmax = @hrslist;
  print "hrslist is $hrslist, num is $hrsmax \n";
  ...
  for ($i=0;$i < $hrsmax / $numcols;$i++) {
    ...
    while ($hour < 24) {
      ...
      for ($hrsidx = $idxstart;$hrsidx < $idxstop;$hrsidx++) {
        $hrs = $hrslist[$hrsidx];
        printf"  %5d", $gdsto{$hrs}{$gds}[$hour]; 
        $total{$hrs} = $total{$hrs} + $gdsto{$hrs}{$gds}[$hour];
        $gdstotal{$gds}[$hour] = $gdstotal{$gds}[$hour] + $gdsto{$hrs}{$gds}[$hour];
      } 
      print "\n";
      $hour++;
    }

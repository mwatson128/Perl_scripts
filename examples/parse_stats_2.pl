...
sub parse_cmdline {
  ...
  while ($opt = shift @ARGV) {
    ...
    # Handle -h option for HRS filter
    if ($opt eq "-h") {

      # If we haven't already tturned on the HRS filter, do so now 
      if ($flag_hrs ne "ON") {
        $filter_hrs = shift @ARGV;
        $flag_hrs = "ON";
      }

      # Otherwise exit with an error message
      else {
        print STDERR "Duplicate $opt option.\n";
        exit; 
      } 
    }
    ...
    # Handle the -d option for setting which dwell time.
      ...
      if ($temp eq "g" || $temp eq "h" || $temp eq "t") {
        $dwell_type = $temp;
      }
    }
    ...
    # Handle the -g option to allow filtering by GDS
      ...
      # If the gds_filter has not ben previously set
        if ($flag_gds ne "ON") {
          $filter_gds = shift @ARGV;
          $flag_gds = "ON";
        }

	# Duplicate -g option, so write an error message and exit.
        else {
          print STDERR "Duplicate $opt option.\n";
          exit; 
        }
    ...
    # Handle the -trq option to allow filtering by request TPE
    elsif ($opt eq "-trq") {

      # If the trq_filter has not ben previously set
        if ($flag_trq ne "ON") {
          $filter_trq = shift @ARGV;
          $flag_trq = "ON";
        }

        # Duplicate -trq option, so write an error message and exit.
        else {
          print STDERR "Duplicate $opt option.\n";
          exit;
        }
    ...
    # Handle the -trp option to allow filtering by request TPE
    elsif ($opt eq "-trp") {

      # If the trp_filter has not ben previously set
        if ($flag_trp ne "ON") {
          $filter_trp = shift @ARGV;
          $flag_trp = "ON";
        }

        # Duplicate -trp option, so write an error message and exit.
        else {
          print STDERR "Duplicate $opt option.\n";
          exit;
        }
...
sub open_stats_file {
  ...
  if ($tail_file eq "TRUE") {
    ...
    open FIL, "/bin/tail -$tail_limit $statfile |" or die "Can't open $statfile for reading.\n";
  }
}

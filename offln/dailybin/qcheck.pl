#!/bin/perl
#*TITLE - qcheck.pl - Perl script to run daily offline processes - 1.8
#*SUBTTL Preface, and environment 
#
#  (]$[) qcheck.pl:1.8 | CDATE=09:25:44 01/05/05
#
#
...
sub qcheck_rcp 
{
  ...
  print(LOGFILE "start CP of ${qcheck_file}: ", `date`);
  if ($rtn_val = system ("cp ${qcheck_dir}/${qcheck_file} . >>$logfile 2>>$logfile")) {
  ...
  print(LOGFILE "end CP of ${qcheck_file}: ", `date`);
  LOGEXIT("QCHECK: RCP");
...
sub qcheck_ksftally 
{
  ...
  if ($rtn_val = system("purge -d ${database_b} -t ${date} -n ksfsize -f ddate  >> $logfile 2>> $logfile")) {

...
@file_cont = `cat $tmpfile`;  #$chn ${rtdir}${filename}`;
...
#message number 0C14F57F5B0ADD in context. =>
  ...
  if ($ln =~ /Cannot find/) {
    ...
    # Get CHN from (SC1-A2)
    ($fld_chn, $sec) = split /-/, $flds1[4];
    @parts = split //, $fld_chn;
    shift @parts;
    $fld_chn = join("", @parts);

    if ($chn eq $fld_chn) {
      $arvtime = $flds1[2] . " " . $flds1[3];
      #print "MSN is $flds1[11] \n";
      $msg_hash{$flds1[11]}[$ARVT] = $arvtime;
    }
  }
}

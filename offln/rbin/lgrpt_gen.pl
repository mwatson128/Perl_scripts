#!/bin/perl
#*TITLE - lgrpt_gen.pl - Perl script to run daily offline processes - 1.20
#*SUBTTL Preface, and environment 
#
#  (]$[) lgrpt_gen.pl:1.20 | CDATE=14:24:11 11/28/06
#
#
...
#
    ...
    #system ("gunzip ${lgrej_file}* >>$logfile 2>>$logfile");
  #}
  ...
  if (-f "./${lgrpt_file}") {
  ...
  elsif (-f "${lgrpt_dir}/${lgrpt_file}.Z" ||
         -f "${lgrpt_dir}/${lgrpt_file}.gz") {
    ...
    print(LOGFILE "start UNCOMPRESS of ${lgrpt_file}: ", `date`);
    if ($rtn_val = 
  ...
  # get USW Plus Report Log
  # 
  # check to see if report log plus is compressed, or even there
  if (-f "./${lgrpt_plusfile}") {
    # nothing to do
  }
  elsif (-f "${lgrpt_dir}/${lgrpt_plusfile}.Z" ||
         -f "${lgrpt_dir}/${lgrpt_plusfile}.gz") {
 
    # RCP the file
    print(LOGFILE "start RCP of ${lgrpt_plusfile}.Z: ", `date`);
    if ($rtn_val = 
     system ("cp ${lgrpt_dir}/${lgrpt_plusfile}.* . >>$logfile 2>>$logfile")) {
    ...
    # uncompress the LGRPT file
    print(LOGFILE "start UNCOMPRESS of ${lgrpt_plusfile}.Z: ", `date`);
    if ($rtn_val = 
        system("guzip ${lgrpt_plusfile} >> $logfile 2>> $logfile")) {
  ...
  elsif (-f "${lgrpt_dir}/${lgrpt_plusfile}") {

    # meanwhile, go ahead, rcp it over, and continue processing as normal
    print(LOGFILE "start RCP of ${lgrpt_plusfile}: ", `date`);
    if ($rtn_val = 
      system ("cp ${lgrpt_dir}/${lgrpt_plusfile} . >>$logfile 2>>$logfile")) {
      # there was an error.  call error handler and exit
      error_out($E_RCPLGRPT, $rtn_val);
...
sub lgrpt_logx 
{
  ...
  if ($rtn_val = system("nice logx -c master.cfg ${lgrpt_file} >> $logfile 2>> $logfile")) {
  ...
  # we can delete the rpt and rej if we get here to sae space.
  system("rm -f ${lgrpt_file} ${lgrej_file}");

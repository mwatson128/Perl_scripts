...
sub chnrej_split 
{
  ...
  while(<ULGINJECT2>) {
	...
	# No number, just usw AALSRQ
      ...
      # Normal 3 character chain
        ...
        if ($l3 =~ /[0-9,A-Z]/ && $front !~ /EUR|RNG|RNH/) {
	  $short_ent = $l1 . $l2;
	}
	else {
	  $short_ent = $front;
      ...
      $chaincode_hash{$short_ent} = $short_ent;
    }
  }
  close(ULGINJECT2);
  system("rm -f $ulginject");
  ...
  foreach $ele (sort keys %chaincode_hash) {
    ...
    $list{$ele} = $ele;
    # create a file for each $ele by greping through $ulginject_new
    $grep_key = "IP\=" . $chaincode_hash{$ele} . ", entity|";
    $grep_key .= "IP\=" . $chaincode_hash{$ele} . "-.2, entity|";
    $grep_key .= "IP\=" . $chaincode_hash{$ele} . ".-.2, entity";
    $fName = $ele . ".lgsplt";
    qx(egrep '$grep_key' $ulginject_new > $fName);

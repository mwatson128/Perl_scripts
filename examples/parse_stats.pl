...
##  (]$[) parse_stats.pl:1.21 | CDATE=12/09/05 16:12:29
##
...
sub parse_stats_file {
  ...
  while ($exit_flag ne "TRUE") {
    ...
    # Process statistics
      ...
      # Split the line into these variables
      (
      ...
      # Based on HRS and GDS filters decide if we need to update the counters.
      # First check to see if both filters are set and we have matches.
      if (($flag_gds eq "ON") && ($flag_hrs eq "ON")) {
        if (($gds =~ /$filter_gds/) && ($hrs =~ /$filter_hrs/)) {
          update_counters;  
        }
      }

      # Check that the GDS filter is on and we have a match 
      elsif (($flag_gds eq "ON") && ($gds =~ /$filter_gds/)) {
        update_counters;
      }

      # Check that the HRS filter is on and we have a match.
      elsif (($flag_hrs eq "ON") && ($hrs =~ /$filter_hrs/)) {
        update_counters;
      } 

      # If both filters are off update the counters.
      elsif (($flag_gds ne "ON") && ($flag_hrs ne "ON")) {
        update_counters;
      }
    }
...
sub update_counters {
  ...
  elsif ($rpt =~ /ERRRP|ERRREP|PALSRQ|RPINRQ|PRINRQ|AALSRQ|BOOKRQ|DELETE/) {
    $sum{ERRR} += $num_msg;
...
sub dump_stats {
  ...
  if ($lines_per_header > 0 || 0 == num_iters) {
...
sub init_stats {
  ...
  if ($ENV{LOGNAME} =~ /^usw$|^prod_sup$/) {
    $statdir = "/$zone/prod/logs/stats";  
    $delta_time = 60;
    }
  elsif ($ENV{LOGNAME} eq "qa") {
    $statdir = "/$zone/qa/logs/stats";
    $delta_time = 10;
  }
  elsif ($ENV{LOGNAME} eq "uat") {
    $statdir = "/$zone/uat/logs/stats";
    $delta_time = 10;
  }
  elsif ($ENV{LOGNAME} eq "qa2") {
    $statdir = "/$zone/qa2/stats";
    $delta_time = 10;
  }
  elsif ($ENV{LOGNAME} eq "uswrpt") {
    # these vars are set in cmdline for uswrpt.
  }
  else {
    $statdir = ".";
    $delta_time = 60;
...
sub parse_cmdline {
  ...
  # Handle -s option to show average, max and min values
  if ($opt_s) {
    $summary_flag = "TRUE";
  }

  # Handle -l option for number of lines between headers
  if ($opt_l) {
    $lines_per_header = $opt_l;
  }

  # Handle the -d option for setting which dwell time.
  if ($opt_d) {
    # If we have a valid dwell_type replace it
    if ($opt_d eq "g" || $opt_d eq "h" || $opt_d eq "t") {
      $dwell_type = $opt_d;
    }
  }
    
  # Handle -r option to show message rates vs. message counts
  if ($opt_r) {
    $show_rates = "TRUE";
  }

  # Handle -U option to unlock $tail_limit
  if ($opt_U) {
    $tail_limit = $opt_U;
  }

  # Handle -t option to run in tail mode
  if ($opt_t) {
    $tail_file = "TRUE";
  }

  # Handle the -g option to allow filtering by GDS
  if ($opt_g) {
  ...
  # Unknown command line option, so write an error message and exit.
  else {
    print STDERR "Invalid command line option -$opt-.\n";
    print STDERR "Usage:\n";
    print STDERR "  MM/DD = Date of Stats file to parse\n";
    print STDERR "  or MM/DD/YY = otherwise, current year assumed\n";
    ...
    print STDERR "  -U = number of lines to tail, A for all day\n";
    exit;
  }
  ...
  if ($ENV{LOGNAME} eq "uswrpt") {
    $statdir = "/$zone/loghist/uswprod01/stats/${month}${year}";
    $delta_time = 60;
  }
...
sub open_stats_file {
  ...
  $statfile = sprintf "%s/stats%s.log", $statdir, $filedate;

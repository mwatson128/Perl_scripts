...
print "output is in /$zone/prod/timeout_rpt/$xlResultsName\n";

zip_email();

`mv $xlResultsName /$zone/prod/timeout_rpt/`;
`rm $tmpfile`;
...
sub cmdline() {
  ...
  if ($email_add) {
    $emails = "pedpg\@dhisco.com $email_add"; 
    #$emails = "mike.watson\@dhisco.com $email_add"; 
  }
  else {
    $emails = "pedpg\@dhisco.com";
...
sub zip_email() {
  ...
  qx(uuencode $xlResultsName $xlResultsName | $mailer -s "$sub" $emails);
...
sub fillInTabs {
  ...
  foreach $elem (sort keys %tmoMsgHByDate) {                                      
    ...
    $timeOuts->write($row, $col++, $tmoMsgHash{$msn}[$SGA], $string_format);    
  ...
  if ($atLeastOneTO) {
    ...
    $tmCounts->write('A1', "Time", $string_format);
    $tmCounts->write('A2', "SGA", $string_format);
    $tmCounts->write('C1', "Counts", $string_format);

    $toColNum = $toRowNum = 1;
    foreach $key (reverse sort keys %sgaTMOHashSorted) {
      ($sga, $sgaCnt) = split /_/, $sgaTMOHashSorted{$key};
      $tmCounts->write($toRowNum, $toColNum++, $sga, $string_format);
    }
    $toRowNum++;
  }
  ...
  if ($atLeastOneDB) {
    ...
    foreach $key (reverse sort keys %sgaDSTBHashSorted) {
      ...
      $dbCounts->write($dbRowNum, $dbColNum++, $sga, $string_format);
    }
    $dbRowNum++;
  }
  ...
  if ($atLeastOneDN) {
    ...
    foreach $key (reverse sort keys %sgaDSDNHashSorted) {
      ...
      $dnCounts->write($dnRowNum, $dnColNum++, $sga, $string_format);
    }
    $dnRowNum++;
  }
...
sub populateSgaNames {
  ...
  $fname = "/$zone/prod/config/sganame.cfg";
  open IFP, "< $fname";

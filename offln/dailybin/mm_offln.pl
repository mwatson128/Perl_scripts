#!/bin/perl
#*TITLE - mm_offln.pl - Perl script to run daily offline processes - 1.1
#*SUBTTL Preface, and environment 
#
#  (]$[) mm_offln.pl:1.1 | CDATE=20:13:04 25 Aug 1998
#
#
...

{
  ...
  #
  setenv("database_a", "uswmm");
  setenv("database_b", "uswmm");
  setenv("logxerr_size", "400000");
  
  # production name & file locations
  #
  setenv("remotehost", "mm"); 
  setenv("remotelogin", "uswrpt"); 
  setenv("lgrpt_dir", "/prod/m3/usw/lg");
  setenv("lgrpt_altdir", "/prod/m4/loghist");
  setenv("lgrej_dir", "/prod/m3/usw/lg");
  setenv("lgper_dir", "/prod/m3/usw/lg2");
  setenv("qcheck_dir", "/prod/m2/usw/perf/qdata");
  
  # archive directories for files & reports
  #
  setenv("arcdir_sum", "/reports/daily/sum");
  setenv("arcdir_vol", "/reports/daily/vol");
  setenv("arcdir_per", "/reports/daily/per");
  setenv("arcdir_logxerr", "/reports/daily/logx_err");
  setenv("arcdir_opsrpt", "/reports/daily/opsrpt");

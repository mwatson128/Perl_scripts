...
$emlfn = sprintf "%s_%s.tmp", $lc_chn, $md_hm;
$tmpfn = sprintf "%s_%s.txt", $lc_chn, $md_hm;
...
if ( $ksfstats{"Unsent"} || ($ksfstats{"Pending delivery"} &&
  ...
  print "There are $msg_cnt messages on the $uc_chn queue at $ct.\n";
  ...
  print "There are NO messages on the $uc_chn queue at $ct \n";
}
...
sub stats {
  ...
  if ($?)  {    # if "Child Error" from sfcon
    print "  Received a 'Child Error' from sfcon for chn = $uc_chn \n";
    print "    Make sure the above is a valid chain code and rerun. \n";
    exit 1;
  }
  else {
    chomp(@out);
...
sub call_sfcon {
  $qid = "SFCON_PULL";    
  system "kqcreate $qid";
  @out = `echo "$input" | sfcon -p $qid $queue_name 2> /dev/null`;
}

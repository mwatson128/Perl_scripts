...
#
# (]$[) daybill.pl:1.11 | CDATE=04/02/07 10:54:47
...
$LOG = ">> /uswsup01/usw/reports/daily/billing/daily.log";
...
if ( $ARGC < 1) {
  ...
  `rm /uswsup01/usw/reports/daily/billing/??????.daily`;
  $do_current = true;
}
...
$DATAFILE = "</uswsup01/usw/reports/daily/billing/maillist.cfg";
#$DATAFILE = "</uswsup01/usw/reports/daily/billing/chn.cfg";  # for testing purposes
...
foreach $chain ( sort (keys %chain_codes)) {
  ...
  $OUTFILE = ">>/uswsup01/usw/reports/daily/billing/data/$fname.bil";
  ...
  printf OUTFILE "%s %8d   %6d   ", $tot_day, $bill_all[1], $bill_all[0];
  foreach $gds (@gds_list) {
...
if ($opt_m) {
  ...
  foreach $name_split ( sort( keys %config_file )) {
    ...
    # Don't send email for dummy user
    next if ($user[0] eq "DUMMY");
    ...
    for ($i = 2; $i <= $#user; $i++) {
      ...
      $chn_file = qx(tail -14 /uswsup01/usw/reports/daily/billing/data/"$fname".bil);
...
# /uswsup01/usw/reports/daily/billing so other programs
# know that this process has finished.
if ($do_current) {

  $semdate = `/uswsup01/usw/offln/bin/getydate`;
  `touch /uswsup01/usw/reports/daily/billing/$semdate.daily`;

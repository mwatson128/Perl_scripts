...
sub lgrpt_ptally 
{
  ...
  if ($rtn_val = system("expire -d ${database_b} -t ${edate} -n bperf -f from >> $logfile 2>> $logfile")) {
  ...
  if ($rtn_val = system("purge -d ${database_b} -t ${date} -n aperf -f fromars >> $logfile 2>> $logfile")) {
  ...
  if ($rtn_val = system("purge -d ${database_b} -t ${date} -n bperf -f from >> $logfile 2>> $logfile")) {
...
sub lgrpt_optally 
{
  ...
  if ($rtn_val = system("purge -d ${database_b} -t ${date} -n boperf -f ddate >> $logfile 2>> $logfile")) {
  ...
  if ($rtn_val = system("optally -d ${database_b} -t ${date} >> $logfile 2>> $logfile")) {

#!/usr/local/bin/perl 
##
## (]$[) dt_tally.pl:1.12 | CDATE=02/17/04 17:25:11                          
##                                                        
##                                                        
...
sub setup_environment {
  ...
  $ENV{INFORMIXSERVER} = "usw_dss";

  # Using the date calculate the base value for the decimal UTC 
  $date_d = `/usw/offln/bin/tstamp -t $date -od`;
  chomp $date_d;
  
  # Open ISQL pipe
  #open ISQL, "| /informix/bin/isql usw_perf";
  open ISQL, ">tmp.file";
  ...
  printf ISQL "delete from downtime where time_d >= %d and time_d < %d\n",
         $date_d, ($date_d + 86400);
...
sub parse_statsub {
  ...
  while ($line=<STATSUB>) {
    next if ($line =~ /^$|^#|^0_0_NCD|NCDV|TCPSES/);
    if ($line =~ /^ANS/) {
      $count++;
      @line = split /\//, $line;
      @node = split /_/, $line[0];
      ...
      DBUG_PRINT("parse_statsub", "%s = %s", "\$iplocation{\$ent}", 
           $iplocation{$ent});
      DBUG_PRINT("parse_statsub", "Entity list by IP:");
    }
  }
  close STATSUB;
...
sub output_outages {
  ...
  for $outage (@outages) {
    ...
    DBUG_PRINT("ouput_outages", "%s %s - %s mins: %d", 
               $line[2], $time_down, $time_up, $minutes_down);
  }
...
sub set_connection_up {
  ...
  # If we don't have a downtime handle with care
    ...
    if ($last_up{$ip} ne "") {
      DBUG_PRINT("%s has no downtime and was already reported up at %s", 
           $ip, $last_up{$ip});
    }  
...
sub load_into_db {
  ...
  printf ISQL "load from /tmp/downtime.%s\n", $$;
  print ISQL "insert into downtime\n";
  ...
  while ($block=<RTLOG>) {
    ...
    for $line (@block) {
      ...
      if ($line =~ /A2IPENDN/) {
        ...
        # Grab comm engine name from line 
        $line =~ /TSESLOST\):.* \((.*)\) =>$/;
        $comm_engine = $1;

        DBUG_PRINT("parse_rtlog", "%s DOWN", $comm_engine);
        ...
        DBUG_PRINT("parse_rtlog", "%s back UP", $comm_engine);
  
        # Loop through array of IP for comm engine and set connections up
        # that went down as a result of the CE outage.
        for $ip (@{$iploc{$comm_engine}}) {
          set_connection_up ($ip, $log_time, "CE");
        }
      }
      elsif ($line =~ /halt the system|subprocesses have halted/) {
        # TPE outage, set it all down!
        for $comm_engine (sort keys %iploc) {
          for $ip (@{$iploc{$comm_engine}}) {
            set_connection_down ($ip, $log_time, "CE");
          }

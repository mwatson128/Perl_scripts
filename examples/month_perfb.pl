#!/bin/perl
###########################################################################
# month_perfb.pl - Script for month end billing for Ultraswitch. 1.5
# (]$[) month_perfb.pl:1.5 | CDATE=11/29/06 14:44:45
###########################################################################
#
...
qx(perfb -m > $PERFBDIR/all$prev_mon.perfb); 
...
while (<CHAINFILE>) {
  ...
  qx(perfb -m -h "$hrs_list" > $PERFBDIR/$hrs_prime$prev_mon.perfb);
...
foreach $gds (values %all_gds) {
  ...
  qx(perfb -m -a "$gds" > $PERFBDIR/gds/$lc_gds$prev_mon.perfb);

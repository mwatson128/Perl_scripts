#!/bin/perl
#*TITLE - chainrej.pl - Perl script to email rejected message count and example - 1.16
#*SUBTTL Preface, and environment
#
#  (]$[) chainrej.pl:1.16 | CDATE=13:59:25 11/28/06
#
...
if ($ARGV[1]) {
  ...
  $LOGDIR = "/$zone/usw/offln/daily";
}
$COMPRESSEDLOGDIR = "/$zone/loghist/rej";
...
if(!(open(FILE, $FILENAME))) {
  system("cp ${COMPRESSEDLOGDIR}/rej$date.lg.gz ${RUNDIR}");

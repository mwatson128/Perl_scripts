#!/usr/local/bin/perl
##
## (]$[) dt_tally.pl:1.25 | CDATE=03/21/08 13:01:32                          
##                                                        
##                                                        
...
sub set_defaults {
  ...
  $rtlog_rootdir = "/$zone/loghist/uswprod01/rtlogs";
  ...
  $dist_list = 'pedpg@pegs.com Jen.Shepherd@pegs.com';
...
sub parse_rtlog {
  ...
  while ($block=<RTLOG>) {
	...
	$ent = $1 . "_1";
      ...
      elsif ($line =~ /halt the system|halting system|subprocesses have halted/) {

...
$RELOAD        = "$path/RELOAD-graph";
$DEBUG         = "$path/DEBUG-graph";
$debugout      = "$path/debug.graph";
...
sub SmoothRealTime {
  ...
  foreach $HOST (@hosts) {
    ...
    if ($kidpid{$HOST} = fork) {
      ...
      if ($DO_DEBUG_LOG) { print LOG "[" . strftime("%T", gmtime) . "] Generating graph for $HOST (peak datapoint is $hostmax)...\n"; }
...
#      if ($DO_DEBUG_LOG) { print LOG "[" . strftime("%T", gmtime) . "] Smooth file for $HOST finished.\n"; }
  ...
  ## Debug - print current status/results
  if ($DO_DEBUG_LOG) { print LOG "[" . strftime("%T", gmtime) . "] Generating graph for $env (peak datapoint is $MAX)...\n"; }

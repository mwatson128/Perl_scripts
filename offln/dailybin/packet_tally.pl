#!/usr/local/bin/perl
# (]$[) packet_tally.pl:1.11 | CDATE=01/18/08 08:48:14
############################################################
...
@machine_list = qw ( usw_prod sunpa sunpb suntcp3 suntcp4 suntcp5 
		     suntcp6 suntcp7 suntcp8 sunads1 sunads2 sunads3
                   );

@short_machine_list = qw ( prod sunpa sunpb tcp3 tcp4 tcp5 tcp6 tcp7
			   tcp8 ads1 ads2 ads3
                         );
...
$root_dir = "/$zone/loghist/uswprod01/netstat";
...
for $machine (@machine_list) {
  ...
  # Different directories between TPE and CE
  if ($machine eq "usw_prod") {
    $RCP_CMD = sprintf "/usr/local/bin/scp usw\@usw_prod:/uswprod01/perf/kv/%s/netstat.%s %s/%s/netstat.usw_prod.%s", $monthdir, $filedate, $root_dir, $monthdir, $filedate; 
  }
  else {
    $RCP_CMD = sprintf 
         "/usr/local/bin/scp usw\@%s:/usr2/usw/perf/kv/%s/netstat.%s %s/%s/netstat.%s.%s", $machine, $monthdir, $filedate, $root_dir, $monthdir, $machine, $filedate; 
  }
  system $RCP_CMD;
}
